...
import { Flow, ErrOption } from './Flow.js';

export class FlowImpl {
	private _p: Promise<any>;
	private _sc: SeleniumWrapper;
	// private _waitTime: number = 60 * 1000;
	private _flDebug: boolean = false;
	// private lastUrl: string = '';
	// private _opt: FlowOption;
	private _flow: Flow;
	...
	strToPom(str: string): PomStructure {
		if (str.startsWith("//", 0)) return { name: "", xpath: str };
		if (str.startsWith("/")) return { name: "", xpath: str };
		if (str.startsWith("(")) return { name: "", xpath: str };
		return { name: "", css: str };
	}
	...
	getSelenium(): SeleniumWrapper {
		return this._sc
	}

	comment(msg: String): Flow {
	...
	combine(reusable: BaseReusable, opts: Array<any> = []): Flow {
	...
	delaySec(n: number): Flow {
		this.delay(n * 1000);
		return this._flow;
	}

	delayMin(n: number): Flow {
		let i: number = 0;
